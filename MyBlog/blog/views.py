...
def blog_list(request,cid=-1, tid=-1, pageNum =-1):
    post_list = None
    if cid != -1:
    ...
    #进行分页
    my_paginator = Paginator(post_list, 1)  # 实例化一个分页对象, 每页显示10个
    page = 0
    if pageNum == -1:
        page = request.GET.get('page', default=1)  # 从URL通过get页码，如?page=3
    else:
        page = request.GET.get(pageNum)
    # 获取某页对应的记录, 如果
    post_listA = my_paginator.get_page(page)
